<div class="admin-sidebar-top">
    <a href="/admin"><img src="{{asset('backend/asset/images/logo.png')}}" alt=""></a>
                    
                </div>
                <div class="admin-sidebar-content">
                    <ul>
                        <li><a href=""><i class="ri-dashboard-line"></i> Dashboard</a>
                            <ul class="sub-menu">
                                <div class="sub-menu-item">
                                    <li><a href="/admin"><i class="ri-arrow-drop-right-line"></i>Tổng quan</a></li>
                                </div>
                            </ul>
                            </li>
                        <li><a href=""><i class="ri-file-list-3-line"></i> Đơn hàng</i></a>
                            <ul class="sub-menu">
                                <div class="sub-menu-item">
                                    <li><a href="/admin/order/list"><i class="ri-arrow-drop-right-line"></i>Danh sách</a></li>
                                    <li><a href=""><i class="ri-arrow-drop-right-line"></i>Thêm</a></li>
                                </div>
                            </ul>
                        </li>
                        <li><a href=""><i class="ri-menu-line"></i> Sản phẩm</i></a>
                            <ul class="sub-menu">
                                <div class="sub-menu-item">
                                    <li><a href="/admin/product/list"><i class="ri-arrow-drop-right-line"></i>Danh sách</a></li>
                                    <li><a href="/admin/product/create"><i class="ri-arrow-drop-right-line"></i>Thêm mới</a></li>
                                </div>
                            </ul>
                        <li><a href=""><i class="ri-calendar-check-line"></i> Đặt lịch làm gốm</a>
                            <ul class="sub-menu">
                                <div class="sub-menu-item">
                                    <li><a href="/admin/book/list"><i class="ri-arrow-drop-right-line"></i>Danh sách</a></li>
                                </div>
                            </ul>
                        </li>
                        <li><a href=""><i class="ri-calendar-check-line"></i>Tin tức</a>
                            <ul class="sub-menu">
                                <div class="sub-menu-item">
                                    <li><a href="/admin/news/list"><i class="ri-arrow-drop-right-line"></i>Danh sách</a></li>
                                    <li><a href="/admin/news/create"><i class="ri-arrow-drop-right-line"></i>Thêm mới</a></li>
                                </div>
                            </ul>
                        </li>
                        <li><a href=""><i class="ri-calendar-check-line"></i>Danh mục</a>
                            <ul class="sub-menu">
                                <div class="sub-menu-item">
                                    <li><a href="/admin/category/list"><i class="ri-arrow-drop-right-line"></i>Danh sách</a></li>
                                    <li><a href="/admin/category/create"><i class="ri-arrow-drop-right-line"></i>Thêm mới</a></li>
                                </div>
                            </ul>
                        </li>
</div>